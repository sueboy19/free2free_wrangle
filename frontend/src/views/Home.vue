      ...
      <div class="md:hidden" v-if="authStore.isAuthenticated">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gray-50">
          ...
          <button
            ...
            class="block w-full text-left px-3 py-2 text-gray-700 hover:text-gray-900"
          >
            登出
          </button>
        </div>
    ...
    <main class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="!authStore.isAuthenticated" class="text-center">
        <div class="max-w-2xl mx-auto">
          <h2 class="text-3xl font-bold text-gray-900 mb-4">歡迎來到買一送一配對網站</h2>
          <p class="text-lg text-gray-600 mb-8">
            找尋夥伴一起享受買一送一的優惠！登入後即可開始瀏覽和創建配對。
          </p>
          <router-link to="/login" class="btn-primary text-lg px-8 py-3"> 立即登入 </router-link>
        </div>
      </div>
      ...
      <div v-else>
        ...
        <div class="mb-8">
          ...
          <p class="text-gray-600">今日已有 {{ todayMatchesCount }} 個新的配對機會</p>
        </div>
        ...
        <div class="card">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              ...
              <router-link
                :to="`/matches/${match.id}`"
                class="mt-3 btn-primary w-full text-center block text-sm"
              >
                查看詳情
              </router-link>
            </div>
