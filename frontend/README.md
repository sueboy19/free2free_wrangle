...
本專案使用 **redirect + 短期 code + API 交換** 方式實作 OAuth 登入，確保在移動設備上也能正常運作：
...
7. 前端從 URL query 讀取 code
8. 前端調用 `/auth/exchange-code` API 換取實際的 JWT token
9. 設置 token 並跳轉首頁
...
前端從 URL query 參數讀取 code，然後調用 API 換取 token：
