...
describe('E2E 測試 2：配對容量驗證測試（當前行為）', () => {
  ...
  it('當前行為：參與配對沒有容量限制', async () => {
    // 用戶 1 參與配對
    const user1Result = await joinMatch(env, participant1Token, matchId);
    expect(user1Result.ok).toBe(true);
    expect(user1Result.data).toBeDefined();
    expect(user1Result.data.status).toBe('pending');

    // 用戶 2 參與配對
    const user2Result = await joinMatch(env, participant2Token, matchId);
    ...
    const user3Result = await joinMatch(env, participant3Token, matchId);
  ...
  it('記錄：當前系統缺少容量檢查', async () => {
    // 這個測試只是用於記錄當前系統的行為
    const testResult = await joinMatch(env, participant1Token, matchId);
  ...
  it('開局者不能參與自己的配對', async () => {
    const result = await joinMatch(env, organizerToken, matchId);
  ...
  it('同一用戶不能重複參與同一配對', async () => {
    ...
    // 第一次參與
    const firstResult = await joinMatch(env, participant1Token, newMatchId);
    expect(firstResult.ok).toBe(true);

    // 第二次參與應該失敗
    const secondResult = await joinMatch(env, participant1Token, newMatchId);
